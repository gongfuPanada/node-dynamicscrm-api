...
var Util = function (settings) {
    ...
    var stylesheet                      = xslt.readXsltFile(__dirname + "/templates/remove_ns.xslt");
    ...
    */
    this.Create = function(options, cb)
    {
        ...
        if (auth) {
            var item = self.cacheAuth.get(auth);
            options.KeyIdentifier = item.KeyIdentifier;
            options.CiperValue0 = item.CiperValue0;
            options.CiperValue1 = item.CiperValue1;
    ...
    */
    ...
    var executeSoapPost = function(options, action, template, body, cb)
    {
        ...
        ;
console.log(soapEnvelopeMessage);
        var requestOptions = {
            method: 'POST',
            host: settings.domain + '.api.crm.dynamics.com',
